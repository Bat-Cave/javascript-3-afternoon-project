...
//Code Here
let postTaxPrices = prices.map(num => (num *.07) + num);
...
let orderTotals = orders.map(num => num.price + (num.price*num.tax))
...
let totalArr = [];
let makeArr = purchases.filter(obj => {
  if(obj.owner === "Bob"){
    totalArr.push(obj.price);
  }
});
let bobsTotal = totalArr.reduce((acc, curr) => acc + curr)
